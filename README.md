...
面向中俄贸易操作场景的本地优先桌面工具。当前仓库已完成 M3 审核与安全导出闭环：Electron 桌面端可安全导入 `.xlsx`，经 Python Worker 建立可恢复离线翻译任务，完成人工审核后导出经过重开验证的 Excel 副本。
...
当前不包含在线翻译或本地模型。M4 将完善历史、术语导入导出、设置和自动清理；M5 再完成 Worker 打包、Windows 安装包和真实文件内测。
...
│     └─ renderer/              Excel 预检、离线翻译、审核与导出工作台
...
- Preload 只暴露工作簿、翻译任务、审核导出和 Worker 诊断白名单；
- 后续预览与任务创建只提交临时 `workbookId`，Main 再映射到真实路径；
- Python Worker 标准输出只允许 JSON Lines 协议消息；
- SQLite 只能由 Python Worker 读写，术语和缓存命中只生成待确认候选；
- Renderer 不能提供输出路径；Main 通过原生另存为对话框生成可信目标路径；
- 导出只生成新 `.xlsx` 副本，禁止覆盖原文件或已有文件，并在最终落盘前重开验证；
- 当前不启用在线翻译，也不上传文件或业务数据。

详细协议参见 [IPC 与 Worker 协议](docs/architecture/ipc-protocol.md)，阶段知识参见 [M1 Excel 导入知识快照](docs/architecture/m1-excel-import.md)、[M2 离线翻译任务知识快照](docs/architecture/m2-offline-translation.md) 与 [M3 审核与安全导出知识快照](docs/architecture/m3-review-export.md)。
