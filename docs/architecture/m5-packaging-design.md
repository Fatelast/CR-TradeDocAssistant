...
Beta 的目标是确认安装包具备可交付给有限内测者的基础闭环，而不是替代真实业务发布验收。除既有自动化、解包和 Windows 11 本机安装验证外，仅增加以下两项：

1. 对三份仓库内合成脱敏样本运行 `npm run verify:beta-samples`，该命令校验样本身份、导入映射、人工填写模拟、导出、输出重读、源文件 SHA-256 不变，以及清单要求的样式和公式保留；
2. 在一台装有 Microsoft Excel 或 WPS 的 Windows 电脑上，将任一导出文件人工复开一次，确认可打开且译文写入预期列。

三份演示样本只覆盖基础单表、非首行表头/已有译文和多工作表/样式/公式。它们不作为真实贸易 Excel、双办公软件、Windows 10/11 干净环境或 schema 迁移的替代证据。
...
- 当前仓库有 6 个固定合成 Excel 样本（M1 3 份、M5 Beta 3 份），真实文件验收仍依赖脱敏样本；
