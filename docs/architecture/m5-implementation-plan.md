...
> 状态：M5 工程实现与 Windows 11 本机安装验收完成；Beta 轻量验收技术闭环已通过；V1 外部验收待完成
> 版本：`0.6.0-beta.1`
...
- [x] 增加私有脱敏样本清单模板和验收规范；
- [x] 增加本地 SHA-256、工作簿结构和 Office 结果门禁校验工具；
- [x] 增加 3 份仓库内脱敏演示样本、清单和 Worker 端到端校验命令；
- [ ] 在 Excel 或 WPS 中完成 1 份导出文件人工复开；
- [ ] 记录一次 Beta 轻量验收结论。

> 演示样本只证明基础技术闭环，不替代真实贸易文件，也不要求双办公软件或 16 类矩阵。
